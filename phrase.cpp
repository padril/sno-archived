...
Phrase::Phrase(const char* str) {
	...
	int t = 0;
	auto temp_result = evaluate(phrase.tokens, phrase.literals, t);
	switch (temp_result.type) {
	case LITERAL_INT:
		result = std::to_string(*(int*)temp_result.ptr);
		delete (int*)temp_result.ptr;
		break;
	}
	std::cout << result;
}
...
auto parse_literal_or_variable(const std::string& str, int& pos) {
	...
	// TODO: if it starts with "", dont return a variable type
	...
	if (pos > start) { // there was a letter, its a variable
		...
		return returnType{ Phrase::LITERAL_VAR, new std::string(str.substr(start, pos-- - start)) }; // have to do pos-- here to put pos back to the end of the literal
	}
	else { // no letter, 
...
Tptr OPERATOR_POSITIVE(Tptr arg1) {
	using enum Token;
	switch (arg1.type) {
	case LITERAL_INT: return Tptr{ new int(+*(int*)arg1.ptr), LITERAL_INT }; break;
...
Tptr OPERATOR_ADD(Tptr arg1, Tptr arg2) {
	...
	auto arg1val =	arg1.type == LITERAL_INT ? *(int*)arg1.ptr : (
					/* else */ 0
					);
	auto arg2val =	arg2.type == LITERAL_INT ? *(int*)arg2.ptr : (
					/* else */ 0
					);
...
Tptr Phrase::evaluate(std::vector<Token> token_list, std::list<void*>& literal_list, bool terminating) {
	using enum Token;
	...
	for (; pos < token_list.size(); pos++) {
		...
		case BEGIN_PRIORITY:
			pos++;
			left = evaluate(token_list, literal_list, false);
			break;
		case END_PRIORITY:
			goto end; // this is probably memory unsafe, check later
		case POSITIVE:
			pos++;
			left = OPERATOR_POSITIVE(evaluate(token_list, literal_list, true));
			break;
		case ADD:
			switch (left.type) {
			case LITERAL_INT:
				pos++;
				left = OPERATOR_ADD(left, evaluate(token_list, literal_list, true));
				break;
			}
			break;
		case LITERAL_INT:
	...
	if (pos == token_list.size()) {
		...
		for (Tptr i : used) {
			if (i.ptr != left.ptr) {
				switch (i.type) {
				case LITERAL_INT: delete (int*)i.ptr;
				}
			}
		}
	}
